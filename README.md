# Next Simple (SpiraCSS Sample)

A minimal Next.js sample to try SpiraCSS with a simple, CSS Modules-based setup.
...
- CSS Modules are used for most components, but the About page uses plain string class names (HTML-first)
...
- `src/styles/pages/home.module.scss`
  - Page entry SCSS module
- `src/styles/globals.scss`
  - Global styles shared by all pages (loaded in RootLayout)
- `src/components/layouts/BaseLayout.module.scss`
  - Shared layout container styles
- `src/components/**/` + `scss/`
  - Component SCSS modules and child SCSS files
- `spiracss.config.js`
...
The About page keeps `className="..."` as plain strings so you can treat it like HTML input.
